...
namespace Tessera
{
    ...
    [AddComponentMenu("Tessera/TesseraTilemapOutput")]
    public class TesseraTilemapOutput : MonoBehaviour, ITesseraTileOutput
    {
        ...
        public void UpdateTiles(IEnumerable<TesseraTileInstance> tileInstances)
        {
            ...
            Vector3Int GeneratorCellToMapCell(Vector3Int p)
            {
                if (useWorld)
                {
                    ...
                    var worldP = generator.transform.TransformPoint(generator.GetLocalPosition(p));
            ...
            foreach (var i in tileInstances)
            {
                ...
                var mapTile = ScriptableObject.CreateInstance<TransformTile>();
                if (useWorld)
                {
        ...
        private class TransformTile : Tile
        {
